...
{
    [Autorizar(JwtSoPermissao.Profissional)]
    public class ProfissionalController(IProfissionalService service) : AjaxController
    {
        ...
        [HttpPost]
        public async Task<IActionResult> PostAsync(Profissional model) =>
            await ExecuteAsync(async () =>
            {
                var profissional = await _service.CreateAsync(model);
                return Created($"/{profissional.Id}", profissional);
            });

        [HttpPut("{id}")]
        public async Task<IActionResult> PutByIdAsync(int id, Profissional model) =>
